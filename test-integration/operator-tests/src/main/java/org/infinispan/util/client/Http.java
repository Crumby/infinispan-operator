package org.infinispan.util.client;
...
public class Http {
    ...
    public Http stores(Path keystorePath, String keystorePassword, Path trustStorePath, String trustStorePassword, HostnameVerifier verifier) {
        try {
            SSLContext sslContext = SSLContexts.custom()
                    .loadKeyMaterial(keystorePath.toFile(), keystorePassword.toCharArray(), keystorePassword.toCharArray())
                    .loadTrustMaterial(trustStorePath.toFile(), trustStorePassword.toCharArray()).build();
            sslsf = new SSLConnectionSocketFactory(sslContext, new String[] { "TLSv1", "TLSv1.2" }, null, verifier);
        } catch (IOException | GeneralSecurityException e) {
            throw new RuntimeException("Can't create SSL connection factory", e);
